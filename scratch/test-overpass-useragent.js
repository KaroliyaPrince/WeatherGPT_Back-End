...
async function testOverpassWithUserAgent() {
  ...
  const query = `[out:json][timeout:10];node["place"~"city|town|village|municipality"](${bbox});out body;`;

  for (const ep of OVERPASS_ENDPOINTS) {
    console.log(`Trying Overpass mirror: ${ep} with User-Agent...`);
    const t0 = Date.now();
    try {
      const res = await axios.post(ep, `data=${encodeURIComponent(query)}`, {
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
          'User-Agent': 'WeatherGPT-Route-App/1.0 (contact@weathergpt.app)'
        },
        timeout: 10000
      });
